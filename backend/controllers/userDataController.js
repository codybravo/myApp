import { appUserDataModel } from '../models/model.js';
import mongoose from 'mongoose';

// get all app data
const getAllAppUserData = async (req, res) => {
    const appUserData = await appUserDataModel.find({}).sort({createdAt: -1})
...
const getSingleAppUserData = async (req, res) => {
    ...
    const appUserData = await appUserDataModel.findById(id)
    if(!appUserData){
...
const createAppUserData = async (req, res) => {
    ...
    try {
        const appUserData = await appUserDataModel.create({ email, name, role, sheet_url })
        res.status(200).json(appUserData)
    } catch (error) {
...
const deleteAppUserData = async (req, res) => {
    ...
    const appUserData = await appUserDataModel.findOneAndDelete({ _id: id })
...
const updateAppUserData = async (req, res) => {
    ...
    const appUserData = await appUserDataModel.findOneAndUpdate({ _id: id }, {...req.body})
